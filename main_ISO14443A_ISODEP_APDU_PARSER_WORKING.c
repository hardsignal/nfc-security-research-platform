...
volatile uint8_t apdu_cmd[16];
volatile uint8_t apdu_sw1;
volatile uint8_t apdu_sw2;
volatile uint8_t apdu_inf_len;
volatile uint8_t isodep_pcb;
volatile uint8_t isodep_inf0;
volatile uint8_t isodep_inf1;
...
static void st25_isodep_test(void)
{
    ...
    isodep_tx[0] = 0x02U;
    isodep_tx[1] = apdu_cmd[0];
    isodep_tx[2] = apdu_cmd[1];
    isodep_tx[3] = apdu_cmd[2];
    isodep_tx[4] = apdu_cmd[3];
    ...
    if (isodep_rx_len > 0U)
    {
        ...
        if (isodep_rx_len >= 5U)
        {
            apdu_sw1 = isodep_rx[isodep_rx_len - 4U];
            apdu_inf_len = isodep_rx_len - 3U;
            apdu_sw2 = isodep_rx[isodep_rx_len - 3U];
        }
    }
}
